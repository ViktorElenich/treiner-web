...
| Three.js r147 | 3D-частицы «золотая пыль» в Hero | CDN, лениво после window.load |
| Google Fonts | Oswald, Bebas Neue, Playfair Display, Montserrat | CDN |

Весь JS устойчив к недоступности CDN: если GSAP/Lenis/Three не загрузились, сайт работает без анимаций (guard `hasGsap`), прелоадер скрывается по таймауту (2.5 сек в main.js + аварийный inline-скрипт 4 сек в index.html + `<noscript>`).
...
- **Oswald** (`--font-heading`, вес 500) — заголовки секций, цены. Bebas Neue остался fallback-ом: у него нет кириллицы, из-за чего русские заголовки раньше рендерились системным шрифтом
...
- **Bebas Neue без кириллицы**: русские заголовки рендерились системным fallback-шрифтом, на мобайле тире hero-заголовка переносилось на отдельную строку. Заголовочный шрифт заменён на Oswald (вес 500), Bebas остался fallback-ом для латиницы/цифр
- **Сайт зависал на «LOADING» при недоступном CDN**: `gsap.registerPlugin()` без проверки ронял весь main.js. Добавлен guard `hasGsap` во все анимационные модули, таймаут прелоадера 2.5 сек, аварийный inline-скрипт (4 сек) и `<noscript>` в index.html
- **При `prefers-reduced-motion` статистика показывала нули**: `initCounters()` выходил до записи значений. Теперь без анимаций цифры проставляются мгновенно из `data-count`

## Плейсхолдеры для замены

Все плейсхолдеры заполнены. Webhook подключён к `https://treiner-bot.onrender.com`.
...
- [x] Реальные отзывы с Avito (5 шт: Алексей Белкин, Елена x2, Татьяна, Наталья — все 5 звёзд)
- [x] Баннер набора (открыт/закрыт) с динамическими датами + форма вейтлиста
- [x] Согласие на рекламные рассылки (`pages/consent-ads.html`, 38-ФЗ ст. 18)
- [x] Согласие на использование фото/видео (`pages/consent-photo.html`, ст. 152.1 ГК РФ)
- [x] Фикс: enrollment banner не растягивается на десктопе (inline-flex + fit-content)
- [x] Сессия 2026-07-03: заголовки переведены на Oswald (кириллица), устойчивость к падению CDN, фикс счётчиков при reduced-motion, preload hero-фото + width/height у `<img>` (CLS), проверка `res.ok` в формах
- [x] 3D motion: золотые частицы в Hero (Three.js r147, два слоя глубины, параллакс за мышью, пауза вне вьюпорта, адаптация под светлую тему, отключение при reduced-motion)
- [x] 3D-tilt карточек услуг и тарифов с золотым бликом (десктоп, vanilla JS, класс `.tilt` + `.tilt-glare`)
- [x] Динамические цифры дефицита (в `initEnrollment()`): детерминированы от даты, без рандома при перезагрузке. «Свободно X из 15» убывает 12→2 по циклу набора (при закрытом наборе — «Группа набрана»), «N записались в этом месяце» растёт ~9→50 по дням месяца, «Осталось N мест» — недельный цикл пн 5 → вс 1. Возможный следующий шаг — реальные цифры из бота через `GET /api/stats` с этими формулами как фолбэком
...
- [x] Уведомление в Роскомнадзор об обработке персональных данных (подано 09.04.2026, номер 100246534)
- [x] Сменить webhookSecret — удалён из JS-исходника сайта (cfeee06)
